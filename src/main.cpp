...
#if defined(NDEBUG)
#error "USD Coin cannot be compiled without assertions."
#endif
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK2() || blockValue == 0)
        return 0;
...
{
    RenameThread("unitedstatedollarcrypto-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // USD: We use certain sporks during IBD, so check to see if they are
